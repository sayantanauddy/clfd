...
def train_task(args, task_id, tnet, node, device):
    ...
    if calc_reg:
        ...
        node.update_omega(args=args, 
                          data=data,
                          get_minibatch_fn=get_minibatch,
                          device=device)

        # Save the parameters
        node.update_theta_prev()
...
def train_all(args):
    ...
    # one for each task
    node = MASNODETaskEmbedding(tnet, args.task_emb_dim, args.mas_lambda, explicit_time=args.explicit_time).to(device)
